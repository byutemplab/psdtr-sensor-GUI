...
class TrajectoriesPatternPreviewer extends StatefulHookWidget {
  final pattern;
  TrajectoriesPatternPreviewer({
    required this.pattern,
    Key? key,
  }) : super(key: key);

  @override
  _TrajectoriesPatternPreviewerState createState() =>
      _TrajectoriesPatternPreviewerState();
}

class _TrajectoriesPatternPreviewerState
    extends State<TrajectoriesPatternPreviewer> {
  ...
  void initState() {
    ...
    timer = Timer.periodic(
      Duration(milliseconds: widget.pattern.measurementTime.toInt()),
      (Timer t) => {
        // update the counter to move green points
        setState(() {
          counter += 1;
        })
      },
...
class PatternPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < pattern.trajectories.length; i++) {
      ...
      int idx = counter % pattern.numberOfMeasurements.toInt();
      if (i == hoveredTrajectory && hoveringStart) {
        canvas.drawCircle(greenPoints[idx],
